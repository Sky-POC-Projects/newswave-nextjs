...
export default function PublisherDashboardPage() {
  ...
  useEffect(() => {
    if (!authLoading && userId) {
      // Filter articles by the logged-in publisher's ID
      // In a real app, this would be a fetch call
      const articles = mockArticles.filter(article => article.authorId === userId)
                                   .sort((a, b) => new Date(b.publishDate).getTime() - new Date(a.publishDate).getTime());
      setPublisherArticles(articles);
      setIsLoading(false);
    } else if (!authLoading && !userId) {
      // Handle case where user is not a publisher or userId is missing
      setIsLoading(false);
    }
  }, [userId, authLoading]);
